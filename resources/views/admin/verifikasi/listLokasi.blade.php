		...
		<div class="panel-container show">
			<div class="panel-content">
				<table class="table table-bordered table-hover table-sm table-striped w-100" id="listLokasi">
					...
					<tbody>
						@foreach ($dataRealisasi as $datalokasi)
							<tr>
								<td>{{$datalokasi->id}}</td>
								<td>{{$datalokasi->nama_lokasi}}</td>
								<td class="text-right">{{$datalokasi->luas_lahan}}</td>
								<td class="text-right">
									Mulai {{$datalokasi->mulai_tanam}}<br>
									Akhir: {{$datalokasi->akhir_tanam}}
								</td>
								<td class="text-right">{{$datalokasi->volume}}</td>
								<td class="text-right">
									Mulai: {{$datalokasi->mulai_panen}}<br>
									Akhir: {{$datalokasi->akhir_panen}}
								</td>
								<td  class="text-center">
									<a href="{{route('verification.tanam.showLocation', $datalokasi->id)}}" class="btn btn-icon btn-primary btn-xs" data-toggle="tooltip" data-original-title="Detail Data" >
