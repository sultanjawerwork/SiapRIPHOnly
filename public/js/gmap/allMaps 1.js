...
// Extend the Map object to add a findPolygonIntersections() method
google.maps.Map.prototype.findPolygonIntersections = function () {
	...
	for (var i = 0; i < polygons.length; i++) {
		for (var j = i + 1; j < polygons.length; j++) {
			var polygonA = polygons[i];
			var polygonB = polygons[j];

			// Check if the polygons intersect
			if (google.maps.geometry.poly.intersects(polygonA, polygonB)) {
				// Calculate the intersection points
				var intersectionPoints = google.maps.geometry.poly.intersection(
					polygonA,
					polygonB
				);

				// Add the intersection points to the intersections array
