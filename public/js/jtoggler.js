	...
	$.extend(Toggler.prototype, {
					...
					if ($element.data("jtlabel-success")) {
						if ($element.prop("checked")) {
							$element
								.next()
								.next()
								.text($element.data("jtlabel-success"));
						} else {
							$element
								.next()
								.next()
		...
		generateTwoStateHTML: function () {
			...
			if (!$element.hasClass("jqtoggler-inited")) {
				...
				var $wrapper = $("<label />", {
					class: $.trim(
						"jtoggler-wrapper " + this._defaults.className
					),
				});
				var $control = $("<div />", {
		...
		generateThreeStateHTML: function () {
			...
			if (!$element.hasClass("jqtoggler-inited")) {
				...
				var $wrapper = $("<div />", {
					class: $.trim(
						"jtoggler-wrapper jtoggler-wrapper-multistate " +
							this._defaults.className
					),
				});
				var $control = $("<div />", {
					class: "jtoggler-control",
				...
				$element.wrap($wrapper).after($control);
				$control
					.find(".jtoggler-btn-wrapper:first")
					.addClass("is-active");
			}
		},
